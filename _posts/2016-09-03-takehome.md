...
## Here's the finished [app](https://storage.googleapis.com/q-dev-challenge-hannah.appspot.com/index.html)
## And here's my Github [repo](https://github.com/deenhe91/QloudAnimals)
...
![](https://github.com/deenhe91/deenhe91.github.io/blob/master/images/flow.jpg?raw=true)
...
Firebase, firebase, how you pain me. What I really wanted to do here was fill my firebase iteratively but doing this with python3 turns out to be a bit tricky. In order to get my front-end started I saved all my results in json and manually imported it into my firebase storage on the console itself. This needs to be rectified and will be later. 
...
I started building locally, and checking my progress using a python simply server. This means navigating to the directory where your app files are stored and then typing `$ python3 -m simple.server` on the command line.
...
Then connection to the firebase database happens in the javascript file. You do this using `firebase.database.ref()`, specifying where in your database you want to get to. I had `searchID` so that I could change the input depending on the label that the user would type into the search bar. 
