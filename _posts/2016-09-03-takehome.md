...
#### Overall, the google Vision API appears to do a good job. A notable exception?
...
I started building locally, and checking my progress using a python simply server:

`python3 -m simple.server`

I created `index.js`, `index.html` and `index.css` files. The bulk of interest here was contained in the javascript file. I had to connect to the firebase database
...
Then I had to connect to the firebase database in the javascript file. You do this using `.ref()` and then specify where you want to get to. I had `searchID` so that I could change the input depending on the label that the user would type into the search bar. 
